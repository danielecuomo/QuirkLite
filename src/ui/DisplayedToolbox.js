...
class DisplayedToolbox {
    ...
    findGateAt(pt) {
        ...
        for (let groupIndex = 0; groupIndex < this.toolboxGroups.length; groupIndex++) {
            for (let gateIndex = 0; gateIndex < this.toolboxGroups[groupIndex].gates.length; gateIndex++) {
                let gate = this.toolboxGroups[groupIndex].gates[gateIndex];
    ...
    static _paintGate(painter, hand, gate, rect, isHighlighted, stats, toolboxFillColor=undefined) {
        ...
        drawer(new GateDrawParams(painter, hand, true, isHighlighted, false, false, rect, gate, stats, undefined, [], undefined, toolboxFillColor));
    ...
    _paintDeviations(painter, stats, hand) {
        for (let groupIndex = 0; groupIndex < this.toolboxGroups.length; groupIndex++) {
            ...
            let group = this.toolboxGroups[groupIndex];
            for (let gateIndex = 0; gateIndex < group.gates.length; gateIndex++) {
                if (group.gates[gateIndex] !== undefined) painter.noteTouchBlocker({rect: this.gateDrawRect(groupIndex, gateIndex), cursor: 'pointer'});
            }
        }
    ...
    _paintFocus(painter, stats, hand) {
        ...
        if (f === undefined || (hand.heldGate !== undefined && f.gate.symbol !== hand.heldGate.symbol)) return;
        ...
        let {maxW, maxH} = WidgetPainter.paintGateTooltip(painter, new Rect(0, 0, 500, 300), f.gate, stats.time, true);
        let mayNeedToScale = maxW >= 500 || maxH >= 300;
        painter.ctx.restore();
        let cx = f.rect.right() + 1;
        let hintRect = new Rect(cx, f.rect.center().y, maxW, maxH).snapInside(painter.paintableArea().skipRight(10).skipBottom(20));
        painter.defer(() => WidgetPainter.paintGateTooltip(painter, hintRect, f.gate, stats.time, mayNeedToScale));
    }

    stableDuration(hand) {
        return seq(hand.hoverPoints()).map(p => this.findGateAt(p)).filter(e => e !== undefined).map(e => e.gate.stableDuration()).min(Infinity);
    }

    tryGrab(hand) {
        if (hand.pos === undefined || hand.isBusy()) return hand;
        let f = this.findGateAt(hand.pos);
        if (f === undefined) return hand;
        if (f.gate.symbol === MysteryGateSymbol) setTimeout(() => {
