...
function rearrangeFloatBits(inputTexture, selectedMask, shift=0) {
    return REARRANGE_FLOAT_BITS_SHADER(
        ...
        WglArg.float("selectedMask", selectedMask),
        WglArg.float("shift", shift),
        WglArg.float("wireCount", 1 << currentShaderCoder().float.arrayPowerSizeOfTexture(inputTexture)));
}
...
const REARRANGE_FLOAT_BITS_SHADER = makePseudoShaderWithInputsAndOutputAndCode(
    [Inputs.float('input')],
    Outputs.float(),
    `
    uniform float selectedMask;
...
export {rearrangeBits, rearrangeFloatBits};
