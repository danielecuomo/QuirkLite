...
suite.test("multiQubitGateNeedsAndCreatesNewLiveRow", () => {
    let circuit = new CircuitDefinition(2, [
        new GateColumn([_, Gates.Special.WireCut]),
        new GateColumn([Gates.IsingGates.XX, _])
    ]);

    assertThat(circuit.gateQubitRowsAtColumn(1, 0, Gates.IsingGates.XX)).isEqualTo([0, 2]);
    assertThat(circuit.minimumRequiredWireCount()).isEqualTo(3);
    assertThat(circuit.withMinimumWireCount().numWires).isEqualTo(3);
    assertThat(circuit.gateAtLocIsDisabledReason(1, 0)).isEqualTo(undefined);
});

suite.test("multiQubitGateCannotStartOnCutRow", () => {
    let circuit = new CircuitDefinition(3, [
        new GateColumn([_, Gates.Special.WireCut, _]),
        new GateColumn([_, Gates.IsingGates.XX, _])
    ]);

    assertThat(circuit.gateQubitRowsAtColumn(1, 1, Gates.IsingGates.XX)).isEqualTo([]);
